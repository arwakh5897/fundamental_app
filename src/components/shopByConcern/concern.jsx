    ...
    <div id="ShopByConcern" className="bg-menu rounded ">
      ...
      <div className="grid lg:grid-cols-4 grid-cols-1 gap-4 py-12 px-6 mx-auto text-center">
        {menuData.concerns.map((item, index) => (
          <a key={index} href={item.link} className="bg-buttons text-foreground rounded text-sm font-bold lg:font-medium lg:text-xl p-2 cursor-pointer hover:scale-105 transition">
            {item.name}
          </a>
        ))}
      </div>
